...
By understanding the Hypster object, you can take full advantage of Hypster's powerful configuration management capabilities.# HP Object Methods
...
This example showcases all `HP` methods, including selections from lists and dictionaries with various key types, and using text and number inputs.# Selections, Overrides & Final Vars
...
- If there are no defaults specified, it will raise an error.# The Pythonic API
...
- And much more! :)# Automatic Parameter Naming
...
from hypster import HP, config, save, load
